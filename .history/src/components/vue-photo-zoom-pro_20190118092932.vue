  ...
  <div class="pic-img">
    ...
    >
      ...
      <div
        ...
        :style="[imgSelectorSize,imgSelectorPosition,!outShow && imgSelectorBgSize ,!outShow && imgBgPosition]"
      ></div>
      <div
        v-if="outShow"
        v-show="!hideOutShow"
...
export default {
  ...
  watch: {
    ...
    width() {
      let width = (this.selector.width = this.width);
      this.selector.halfWidth = width / 2;
      const { width, height, left, top } = this.imgInfo;
      this.initSelectorProperty(width, height, left, top);
    },
    screenWidth(val) {
      if (!this.timer) {
  ...
  methods: {
    imgLoaded() {
      ...
      if (JSON.stringify(this.imgInfo) != JSON.stringify(imgInfo)) {
        const { width, height, left, top } = (this.imgInfo = imgInfo);
        this.initSelectorProperty(width, height, left, top);
