...
export default {
  ...
  watch: {
    ...
    width(n) {
      this.selector.width = n;
      this.selector.addWidth = !this.outShow ? (n / 2) * (1 - this.scale) : 0;
      this.initSelectorProperty();
    },
    outShow(n) {
      this.selector.halfWidth = n
        ? 0
        : ((this.width / 2) * (1 - this.scale)) / 2;
    },
    screenWidth(val) {
      if (!this.timer) {
  ...
  methods: {
    ...
    mouseMove(e) {
      if (!this.hideZoom && this.imgLoadedFlag) {
        ...
        let addWidth = halfWidth * (1 - scale);
